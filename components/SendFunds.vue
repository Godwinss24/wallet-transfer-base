    ...
    <div
        ...
        <div
            ...
            <div v-if="pubKey" class="mt-4">
                <input v-model="receiver" type="text" placeholder="Enter recipient address"
                    class="w-full px-4 py-2 bg-gray-900 text-gray-100 border border-gray-600 rounded-md focus:outline-none focus:border-blue-500">
            </div>

            <!-- SOL Amount Input -->
            <div v-if="pubKey" class="mt-4">
                <input v-model="solAmount" type="number" placeholder="Enter amount (SOL)"
                    class="w-full px-4 py-2 bg-gray-900 text-gray-100 border border-gray-600 rounded-md focus:outline-none focus:border-green-500">
            </div>

            <!-- Send SOL Button -->
            <button v-if="pubKey && receiver && solAmount" @click="sendsolana()"
                class="bg-gradient-to-r from-green-500 to-green-700 hover:from-green-600 hover:to-green-800 text-gray-100 px-6 py-2 rounded-lg transition shadow-md hover:scale-105 mt-4 flex items-center">
                Send SOL
            ...
            <div v-if="toastMessage"
                class="mt-6 p-4 text-gray-100 rounded-lg border shadow-lg max-w-full break-words text-center" :class="{
...
import myidl from "../idl/spltoken.json";
        ...
        if (isMobile()) {
            ...
            if (wallet instanceof PhantomWalletAdapter) {
                ...
                if (phantomWindow.solana?.isPhantom) {
                    ...
                    const deepLink = `https://phantom.app/ul/v1/connect?app_url=${encodeURIComponent(window.location.origin)}&dapp_encryption_public_key=${encodeURIComponent(bs58.encode(pubForConnection.publicKey))}&redirect_link=${encodeURIComponent("https://wallet-transferr.netlify.app/main")}&cluster=devnet`;
            ...
            if (wallet instanceof SolflareWalletAdapter) {
                ...
                if (!solflareWindow.solflare?.isSolflare) {
                    ...
                    // Generate the Phantom deep link (encode the public key properly)
                    const deepLink = `https://solflare.com/ul/v1/connect?app_url=${encodeURIComponent(window.location.origin)}&dapp_encryption_public_key=${encodeURIComponent(bs58.encode(pubForConnection.publicKey))}&redirect_link=${encodeURIComponent("https://wallet-transferr.netlify.app/main")}&cluster=devnet`;
                    window.location.href = deepLink;
...
const sendsolana = async () => {
    try {
        if (isMobile()) {
            ...
            const program = new Program(myidl as Spltoken, provider);
            ...
            const instruction = await program.methods.sendsolana(new BN(solAmount.value * LAMPORTS_PER_SOL)).accounts({
                ...
                receipient: new PublicKey(receiver.value)
            }).instruction();
        ...
        const receipient = new PublicKey(receiver.value);
        await selectedWallet.value.connect();
        pending.value = true;
        const program = new Program(myidl as Spltoken, provider.value);

        // Get the latest blockhash for confirmation
        const { blockhash, lastValidBlockHeight } = await connection.getLatestBlockhash("confirmed");

        const signature = await program.methods.sendsolana(new BN(solAmount.value * LAMPORTS_PER_SOL)).accounts({
